...
import static org.mashov.avro.JsonElement.NULL_JSON_ELEMENT;
...
public class ExtendedJsonDecoder extends ParsingDecoder
     ...
     * @return this JsonDecoder
     */
    public void configure(String in) throws IOException {
        if (null == in) {
    ...
    private void injectDefaultValueIfAvailable(JsonParser in, String fieldName) throws IOException {
        ...
        if (field.defaultVal() == JsonProperties.NULL_VALUE) {
            defVal = NullNode.getInstance();
        } else {
            defVal = objectMapper.convertValue(field.defaultVal(), JsonNode.class);
        }
        if (defVal == null) {
            throw new AvroTypeException("Expected field name not found: " + fieldName);
        }
